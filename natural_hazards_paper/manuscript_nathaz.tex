...
%% ── Authors (withheld for double-blind review; see AUTHORS.md) ─────────────
\author*[1]{\fnm{} \sur{Author names withheld for double-blind review}}\email{}

\affil*[1]{Affiliation withheld for double-blind peer review}
...
identify which lakes most urgently require field inspection. Building on
strong regional susceptibility studies, we present the first GLOF
susceptibility model to combine continental scale with strict
cross-national validation, interpretable physical drivers, and an openly
reproducible watch-list, spanning the tropical cordilleras of Ecuador,
Peru and Bolivia to the Patagonian icefields. The
model was trained on 20{,}140 unique glacial lakes derived from 69{,}643
multitemporal lake-year observations (Sentinel-2 and Landsat, 2017--2025)
across fifteen regions in four countries. Of 71 compiled historical GLOFs
(1932--2023), 38 spatially matched the satellite inventory, yielding 34
positive lake-years from 32 distinct source lakes---a severe 1:629 class imbalance
addressed through balanced ensemble learning. Under strict lake-grouped
out-of-fold cross-validation with leakage-free preprocessing, a Random
Forest achieved the highest discrimination (ROC-AUC\,=\,0.828; 95\%~CI:
0.729--0.916; permutation $p<0.001$). Leave-One-Country-Out validation
confirmed geographic transferability (held-out Peru: ROC-AUC\,=\,0.80;
Chile\,=\,0.72). SHAP analysis revealed that GLOF-source lakes are not
simply the largest; they are distinguished by a multivariate signature of
...
50\% in the top 5\%). After isotonic calibration, the scores are
reliable (Brier skill score\,=\,0.24). By converting freely available
satellite data into an openly reproducible, ranked watch-list, this
framework provides resource-constrained Andean authorities with an
actionable tool to prioritize field inspection and protect downstream
communities, directly supporting disaster risk reduction under accelerating
glacier retreat.}
...
public, commercial, or not-for-profit sectors for this work.
...
All authors have read and approved the final manuscript and consent to
its publication.

\bmhead{Availability of data and materials}
The derived glacial-lake inventory, the per-lake feature tables, the
trained models, and all analysis outputs are openly available in a public
project repository (link withheld for double-blind review and provided to
the editors on request); a versioned snapshot will be archived with a
citable DOI (Zenodo) upon acceptance. All primary input datasets are
publicly available from their original providers: Sentinel-2 L2A (ESA
Copernicus, via Google Earth Engine), NASADEM (NASA EARTHDATA), RGI~7.0
glacier outlines (GLIMS/NSIDC), and the compiled historical GLOF inventory
(GloFLOD, INAIGEM, and published tropical-Andes and Patagonian catalogs).
All code, trained models, and derived datasets are released under the MIT
licence in the repository referenced above.

\bmhead{Code availability}
The full leakage-free pipeline, trained models, and analysis notebooks are
openly available under the MIT licence in the public project repository
(link withheld for double-blind review; provided to the editors on request).

\bmhead{Authors' contributions}
Author contributions withheld for double-blind review (see separate title page).
